...
services.safetyNewsInsert = (row) => {
    ...
    queryParams += '&' + encodeURIComponent('numOfRows') + '=' + encodeURIComponent(row);
    ...
    request({
        ...
        if(list) {
            ...
            SafetyNews.findOne({id: jsonData[0].id}, (err, result) => {                
                if(err) throw err;
                if (result == null) {                
                    SafetyNews.insertMany(jsonData, (err) => {
                        if(err) throw err;
                    });
                }
            });
...
services.infectionStatus = (check) => {   
    var start = end;
    if(!check) start = '20200310';    

    var url = 'http://openapi.data.go.kr/openapi/service/rest/Covid19/getCovid19InfStateJson';
    var queryParams = '?' + encodeURIComponent('ServiceKey') + '=' + key;
    queryParams += '&' + encodeURIComponent('pageNo') + '=' + encodeURIComponent('1');
    queryParams += '&' + encodeURIComponent('numOfRows') + '=' + encodeURIComponent('10');
    queryParams += '&' + encodeURIComponent('startCreateDt') + '=' + encodeURIComponent(start);
    ...
    request({
        ...
        if(list) {
            ...
            InfectionStatus.findOne({seq: jsonData[0].seq}, (err, result) => {
                if(err) throw err;     
                if (result == null) {
                    InfectionStatus.insertMany(jsonData, (err) => {
                        if(err) throw err;
                    });
                };
            });
...
services.ageGenderStatus = (check) =>{
    var start = end;
    if(!check) start = '20200310';  
    ...
    request({
        ...
        if(list) {
            ...
            var jsonData = JSON.stringify(listData);
            jsonData = JSON.parse(jsonData);        
            AgeGenderStatus.findOne({seq: jsonData[0].seq}, (err, result) => {
                if(err) throw err;      
                if (result == null) {
                    AgeGenderStatus.insertMany(jsonData, (err) => {
                        if(err) throw err;
                    });
                };
            });
